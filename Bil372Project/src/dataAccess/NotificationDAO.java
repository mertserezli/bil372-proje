...
public class NotificationDAO extends DAO {
		...
		for (ProjectBean p : projects) {
			...
			if (p.getMeeting_dates() != null) {
				for (Date d : p.getMeeting_dates()) {
					if (dateNow.getMonth() == d.getMonth() && dateNow.getYear() == d.getYear()
							&& d.getDay() - dateNow.getDay() <= 7) {
						int day = d.getDay() - dateNow.getDay();
						NotificationBean n = new NotificationBean();
						n.setDate(dateNow);
						n.setNotification("(Project) " + p.getTitle() + " adli projenizin " + d + " tarihli meetingine "
								+ day + " gün kalmistir");
	...
	public static List<NotificationBean> getUpcomingDeadlines(String usernameRequest) throws SQLException {
		...
		for (TaskBean t : tasks) {
			...
			if (d.getYear() == dateNow.getYear() && d.getMonth() == dateNow.getMonth()
					&& d.getDay() - dateNow.getDay() <= 7) {
				int day = d.getDay() - dateNow.getDay();
				NotificationBean n = new NotificationBean();
				n.setDate(dateNow);
				n.setUsername(usernameRequest);
				n.setNotification("(Task) " + t.getTitle() + " adli taskinizin " + d + " tarihli deadline tarihine "
						+ day + " gün kalmistir");
				result.add(n);
			}
		}
